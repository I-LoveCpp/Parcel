...
void SetWindow(const char *name,int c,int w){
    char str[30];
    printf(str,"title %s",name);
    system(str);
    printf(str,"mode con cols=%d lines=%d",w,c);
    system(str);
}
...
void DEBUGMODE()
{
	if(DEBUD_TEMP_TYPE)
	{
		BEGIN_CLOCK = clock();
		atexit(DEBUGMODE);
		DEBUD_TEMP_TYPE = false;
	}
	else
	{
		printf("--------------------------------\nProcess exited after %.4f seconds\n", (clock() - BEGIN_CLOCK) * 1.0 / 1000);
		system("pause");
		DEBUD_TEMP_TYPE = true;
	}
}
